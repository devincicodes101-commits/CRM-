...
export async function updateContractorCommissionStatus(
  invoiceId: string,
  ...
  const { error } = await supabase.from("contractor_commission_invoices").update(update).eq("id", invoiceId);
  if (error) return { error: error.message };
  revalidatePath("/contractor-commissions");
}
