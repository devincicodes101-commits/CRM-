...
export const jobInsertSchema = z.object({
  ...
  contractor_acceptance: z.enum(['pending', 'accepted', 'rejected']).nullable().optional(),
  contractor_pay_percent: z.number().min(0).max(100).nullable().optional(),
  contractor_pay_amount: z.number().min(0).nullable().optional(),
  company_share_amount: z.number().nullable().optional(),
  auction_status: z.enum(['open', 'closed', 'won', 'cancelled']).nullable().optional(),
  auction_start_price: z.number().min(0).nullable().optional(),
  auction_current_bid: z.number().min(0).nullable().optional(),
  auction_ends_at: z.string().datetime().nullable().optional(),
  auction_winning_bid: z.number().min(0).nullable().optional(),
  auction_winning_bidder_id: z.string().uuid().nullable().optional(),
