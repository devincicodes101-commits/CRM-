...
-- Idempotent: won't overwrite steps you've edited (UNIQUE(sequence_type, step)).
...
INSERT INTO public.email_sequences (sequence_type, step, delay_days, subject, body, label) VALUES
  ...
  ('invoice_not_paid', 2, 7, 'Invoice {{invoice_number}} now overdue',
   ...
   'Day 7 overdue')
ON CONFLICT (sequence_type, step) DO NOTHING;
